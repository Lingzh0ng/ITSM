...
public class MemberDetailsActivity extends BaseActivity {
  ...
  @OnClick({ R.id.ivMessage, R.id.ivCall, R.id.ivEml }) public void onClick(View view) {
    switch (view.getId()) {
      case R.id.ivMessage:
        if (TextUtils.isEmpty(phoneNumber)) {
          ToastUtils.showLongSafe(R.string.item_phone_no_exist);
        } else {
      ...
      case R.id.ivEml:
        if (TextUtils.isEmpty(phoneNumber)) {
          ToastUtils.showLongSafe(R.string.item_email_no_exist);
        } else {
          doSendEmailTo(email, "");
        }
        break;
